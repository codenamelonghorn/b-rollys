...
#define DEBUGKEY QUIT_LEVEL_SELECT_COMBO

// timer originally was 90 for some reason, but it seems to have been changed before B-Roll
// well, 3 seconds is pretty short, after all.
...
extern void reset_intro_cutscene(void);
...
s16 level_select_input_loop(void) {
    ...
    gCurrSaveFileNum = 2; // file 4 is used for level select tests
    gCurrActNum = 6;
	
	#if DEVELOP
	print_text_fmt_int(200, 116, "CODE %dK", (SEG_ENGINE       - (int)_mainSegmentEnd  ) / 1024 - 5);
	print_text_fmt_int(200, 100, "ULIB %dK", (SEG_FRAMEBUFFERS - (int)_engineSegmentEnd) / 1024	);
	#endif
...
//#ifndef VERSION_JP
    if (sound == 0x15) {
        play_sound(SOUND_MARIO_SPINJUMP, gDefaultSoundArgs);
    } else if (sound == 0x45) {
        play_sound(SOUND_MARIO_YEEAH, gDefaultSoundArgs);
    }
	sound++;
//#endif
    print_intro_text();
		
	reset_intro_cutscene();
		
	#if (DEBUGSW + DEMO)
	if (gPlayer1Controller->buttonDown == DEBUGKEY)  gDebugLevelSelect = 1;
...
int intro_play_its_a_me_mario(void) {
	...
	play_sound(SOUND_GENERAL_COIN, gDefaultSoundArgs); // from comment in actual source, this is exactly how it was
	reset_intro_cutscene();
	sound = 0;
    return 1;
}
