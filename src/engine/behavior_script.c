...
void cur_obj_update(void) {
	...
	f32 distanceFromLuigi;
    BhvCommandProc bhvCmdProc;
    s32 bhvProcResult;

    // Calculate the distance from the object to Mario.
    if (objFlags & OBJ_FLAG_COMPUTE_DIST_TO_MARIO && (gMarioObject != NULL || gLuigiObject != NULL)) {
        gCurrentObject->oDistanceToMario = dist_between_objects(gCurrentObject, gMarioObject);
		if (gLuigiObject != NULL) {
			distanceFromLuigi = dist_between_objects(gCurrentObject, gLuigiObject);
            distanceFromMario = ( gCurrentObject->oDistanceToMario < distanceFromLuigi) ? gCurrentObject->oDistanceToMario : distanceFromLuigi;
			gCurrentObject->oDistanceToMario = distanceFromMario;
	    } else {
