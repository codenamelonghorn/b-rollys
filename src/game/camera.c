 ...
 * Starts a camera shake triggered by an interaction
 */
void set_camera_shake_from_hit2(s16 shake) {
    switch (shake) {
        // Makes the camera stop for a bit
        case SHAKE_SHOCK:
            set_camera_pitch_shake(64.f, 0x8, 0x8000);
            //set_camera_yaw_shake(random_float() * 64.f, 0x8, 0x8000);
            break;
    }
}
...
f32 calc_y_to_curr_floor(f32 *posOff, f32 posMul, f32 posBound, f32 *focOff, f32 focMul, f32 focBound) {
	...
	// fix
	if (gCurrLevelNum == LEVEL_BOWSER_1) focMul = .9f;
...
s32 update_radial_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    calc_y_to_curr_floor(&posY, 1.f, 200.f, &focusY, 1.f, 200.f);
...
s32 update_parallel_tracking_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    calc_y_to_curr_floor(&marioFloorDist, 1.f, 200.f, &marioFloorDist, 1.f, 200.f);
...
s32 update_fixed_camera(struct Camera *c, Vec3f focus, UNUSED Vec3f pos) {
    ...
    calc_y_to_curr_floor(&focusFloorOff, 1.f, 200.f, &focusFloorOff, 1.f, 200.f);
...
s32 update_boss_fight_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    UNUSED u8 filler[20];
    Vec3f secondFocus;
    Vec3f holdFocOffset = { 0.f, -150.f, -100.f };
...
u8 sDanceCutsceneTable[] = {
    CUTSCENE_DANCE_FLY_AWAY, CUTSCENE_DANCE_ROTATE, CUTSCENE_DANCE_CLOSEUP, CUTSCENE_DANCE_DEFAULT, CUTSCENE_DANCE_DEFAULT,
...
void mode_fixed_camera(struct Camera *c) {
    ...
    if (gCurrLevelNum == LEVEL_BBH) {
        ...
        set_fov_function(CAM_FOV_APP_CI);
	} else {
...
s32 update_behind_mario_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    yawSpeed = 0x800;//32 - absPitch / 0x200;
    //if (yawSpeed < 1) {
    //    yawSpeed = 1;
    //}
    //if (yawSpeed > 32) {
...
s16 update_slide_camera(struct Camera *c) {
    ...
    // Slide mode
        ...
        floorHeight = find_floor(c->pos[0], c->pos[1] + 200.f, c->pos[2], &floor) + 100.f;
...
s32 update_mario_camera(UNUSED struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    focus_on_mario(focus, pos, 100.f, 100.f, gCameraZoomDist, 0x05B0, yaw);
...
s16 update_default_camera(struct Camera *c) {
    ...
    calc_y_to_curr_floor(&posHeight, 1, 200, &focHeight, 1.f, 200);
	...
	if ((sMarioCamState->action & (ACT_FLAG_SWIMMING | ACT_FLAG_METAL_WATER)))
	{
		c->focus[1] = sMarioCamState->pos[1] + 300.f + focHeight;
	}
	else
	{
    ...
    camFloorHeight = find_floor(cPos[0], cPos[1], cPos[2], &cFloor) + 30.f;
    ...
    if (waterHeight != -11000.f) {
        ...
        if (!(gCameraMovementFlags & CAM_MOVE_METAL_BELOW_WATER) && camFloorHeight < waterHeight && c->mode == CAMERA_MODE_WATER_SURFACE) {
    ...
    if (sMarioCamState->action & ACT_FLAG_ON_POLE) {
        camFloorHeight = gMarioStates[0].usedObj->oPosY + 80.f;
	...
	if (c->mode != CAMERA_MODE_UNDERWATER) {
        if (dist > 50.f) {
            dist = 50.f;
...
s32 update_spiral_stairs_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    calc_y_to_curr_floor(&focusHeight, 1.f, 200.f, &focusHeight, 1.f, 200.f);
    focus[0] = sMarioCamState->pos[0];
...
void update_camera(struct Camera *c) {
    ...
    if (gCurrLevelNum != LEVEL_CASTLE) {
		...
		if ((sp20->room == 1) && 
			(gMarioStates[0].action == ACT_READING_AUTOMATIC_DIALOG || 
			 gMarioStates[0].action == ACT_WARP_DOOR_SPAWN || 
			 c->cutscene == CUTSCENE_DOOR_PUSH ||
			 c->cutscene == CUTSCENE_DOOR_PULL ||
			 c->cutscene == CUTSCENE_DOOR_PUSH_MODE ||
			 c->cutscene == CUTSCENE_DOOR_PULL_MODE)) {
			set_camera_mode(c, 13, 0);//castle inside mode
		} 
		else if ((sp20->room == 7) && 
			(c->cutscene == CUTSCENE_DOOR_PUSH ||
			 c->cutscene == CUTSCENE_DOOR_PULL ||
			 c->cutscene == CUTSCENE_DOOR_PUSH_MODE ||
			 c->cutscene == CUTSCENE_DOOR_PULL_MODE) &&
			 gMarioObject->oPosZ > -433) {
			set_camera_mode(c, 13, 0);//castle inside mode
		} 
		//else 
			//if (sp20->room != 1) {
			//	set_camera_mode(c, 16, 0);
			//}
    }
...
char gHasShownDialog33Already = FALSE;
void init_camera(struct Camera *c) {
    ...
    switch (gCurrLevelNum) {
        ...
        case LEVEL_CASTLE_GROUNDS:
			if (!gHasShownDialog33Already) {// && !gWarpTransition.isActive) {
				start_cutscene(gCamera, CUTSCENE_MARIO_JOINS);
				gHasShownDialog33Already = TRUE;
			}
    ...
    if (c->mode != CAMERA_MODE_BEHIND_MARIO) {
        c->pos[1] = find_floor(sMarioCamState->pos[0], sMarioCamState->pos[1] + 100.f,
                               sMarioCamState->pos[2], &floor) + 90.f;//125.f;
    }
    vec3f_copy(c->focus, sMarioCamState->pos);
       ...
       // }
        ...
        switch (sMarioCamState->action) {
            ...
            case ACT_STAR_DANCE_EXIT:
                cutscene = CUTSCENE_DANCE_DEFAULT;//determine_dance_cutscene(c);
                break;
...
void set_focus_rel_mario(struct Camera *c, f32 leftRight, f32 yOff, f32 forwBack, s16 yawOff) {
    ...
    calc_y_to_curr_floor(&focFloorYOff, 1.f, 200.f, &focFloorYOff, 1.f, 200.f);
...
void set_fixed_cam_axis_sa_lobby(UNUSED s16 preset) {
    switch (gCurrLevelArea) {
        ...
        case AREA_CASTLE_LOBBY:
            vec3f_set(sFixedModeBasePosition, -656.f, 125.f, 1413.f);
            break;
    }
}
...
BAD_RETURN(s32) cam_castle_lobby_entrance(UNUSED struct Camera *c) {
    vec3f_set(sCastleEntranceOffset, -2013.f - sFixedModeBasePosition[0],
              378.f - sFixedModeBasePosition[1], 1113.f - sFixedModeBasePosition[2]);
}
...
struct CameraTrigger sCamCastle[] = {
	// bowser doors
    { 1, cam_castle_close_mode, -1519, 574, -768, 300, 150, 300, 0 },
    { 1, cam_castle_enter_lobby, -1518, 574, -223, 300, 150, 300, 0 },
	//end bowser doors
	// wf door
    { 1, cam_castle_close_mode, -2571, -36, -1746, 140, 150, 140, 0 },
    { 1, cam_castle_close_mode, -2571, 145, -778, 140, 150, 140, 0 },
    { 1, cam_castle_enter_lobby, -2571, 145, -214, 140, 150, 140, 0 },
	// end wf door
    //! Sets the camera mode when leaving secret aquarium
    { 1, cam_castle_close_mode, 2816, 1200, -256, 100, 100, 100, 0 },
	// lll door
    { 1, cam_castle_close_mode, -455, -36, -1746, 140, 150, 140, 0 },
    { 1, cam_castle_close_mode, -455, 145, -766, 140, 150, 140, 0 },
    { 1, cam_castle_enter_lobby, -455, 145, -224, 140, 150, 140, 0 },
	// end lll door
	// hall leading to courtyard doors
    { 1, cam_castle_close_mode, -1518, 145, -2889, 140, 150, 140, 0 },
    { 1, cam_castle_close_mode, -1208, 145, -257, 140, 150, 140, 0x6000 },
    { 1, cam_castle_enter_lobby, -1044, 145, -159, 140, 150, 140, 0 },
    { 1, cam_castle_close_mode, -1825, 145, -257, 140, 150, 140, -0x6000 },
    { 1, cam_castle_enter_lobby, -1983, 145, -159, 140, 150, 140, 0 },
	// end hall leading to courtyard doors
    ...
    { 1, cam_castle_close_mode, -1516,  715, -4497, 140, 150, 140, 0x4000 },
	// ccm door
    { 1, cam_castle_close_mode, -3746, 350,  591, 140, 150, 140, 0x4000 },
    { 1, cam_castle_enter_lobby, -3204, 350, 590, 140, 150, 140, 0x4000 },
	// end ccm door
	// ddd door
    { 1, cam_castle_enter_lobby, 178, 350, 591, 140, 150, 140, -0x4000 },
    //! Duplicate camera trigger outside JRB door
    { 1, cam_castle_enter_lobby, 178, 350, 591, 140, 150, 140, -0x4000 },
    { 1, cam_castle_close_mode, 722, 350, 590, 140, 150, 140, 0x4000 },
	// end ddd door
    ...
    //{ 1, cam_castle_lobby_entrance, -1023, 376, 578, 300, 400, 300, 0 },
...
s32 rotate_camera_around_walls(struct Camera *c, Vec3f cPos, s16 *avoidYaw, s16 yawRange) {
    ...
    for (step = 0; step < 8; step++) {
        ...
        if (find_wall_collisions(&colData) != 0) {
            ...
            camera_approach_f32_symmetric_bool(&fineRadius, 250.f, 20.f);
...
BAD_RETURN(s32) cutscene_dance_default_focus_mario(struct Camera *c) {
    focus_in_front_of_mario(c, 0.f, 0.4f);
}
...
BAD_RETURN(s32) cutscene_bowser_arena_set_focus(struct Camera *c) {
	...
	set_focus_rel_mario(c, 0, 200.f, 0, 0);
}
...
BAD_RETURN(s32) cutscene_bowser_arena_start(struct Camera *c) {
    ...
    //vec3f_set(sCutsceneVars[0].point, 0.f, 120.f, 1800.f);
    ...
    set_fov_function(CAM_FOV_APP_40);
...
BAD_RETURN(s32) cutscene_bowser_arena(struct Camera *c) {
    ...
    if (gSecondCameraFocus != NULL) {
        ...
        cutscene_event(cutscene_bowser_arena_start, c, 0, 90);
...
// Mario enter game cutscene. By iProgramInCpp.
int cutscene_mario_join_flags = 0;
// Bug fix: you can activate the stage select menu while cutscene is running
s8 canUseStageSelect = TRUE;

void reset_intro_cutscene(void) {
	gHasShownDialog33Already = FALSE;
	cutscene_mario_join_flags = 0;
	canUseStageSelect = TRUE;
}

#define MARIOJOINCUTS_FLAG_FIRST_FRAME 0x80000000
#define MARIOJOINCUTS_FLAG_FIRST_FRAME_OF_SWITCH 0x40000000
#define default_timer 5
int tmr   = default_timer;
BAD_RETURN(s32) cutscene_mario_join_spin2(struct Camera *c) {
    f32 dist;
    s16 pitch, yaw;
	if (!(cutscene_mario_join_flags & MARIOJOINCUTS_FLAG_FIRST_FRAME)) {
		hurb_coeff = HURB_COEFF;
		vec3f_copy(c->focus, sMarioCamState->pos);
		cutscene_mario_join_flags |= MARIOJOINCUTS_FLAG_FIRST_FRAME;
		vec3f_get_dist_and_angle(c->focus, c->pos, &dist, &pitch, &yaw);
		focus_on_mario(c->focus, c->pos, 125, 125, 255.f, pitch, yaw);
		dist = 300.f;
		yaw += 0x8000;
		vec3f_set_dist_and_angle(c->focus, c->pos, dist, pitch, yaw);
		tmr = default_timer;
...
BAD_RETURN(s32) cutscene_mario_join_spin(struct Camera *c) {
    // Function, camera, starting frame, ending frame (-1 to play every frame after 5)
	canUseStageSelect = FALSE;
    cutscene_event(cutscene_mario_join_spin2, c, 0, -1);
}
BAD_RETURN(s32) cutscene_mario_join_switch_to_c_up(struct Camera *c) {
    f32 dist;
    s16 pitch, yaw;
	if (!(cutscene_mario_join_flags & MARIOJOINCUTS_FLAG_FIRST_FRAME_OF_SWITCH)) {
		cutscene_mario_join_flags |= MARIOJOINCUTS_FLAG_FIRST_FRAME_OF_SWITCH;
		gCameraMovementFlags |= CAM_MOVING_INTO_MODE;
		gCameraMovementFlags &= CAM_MOVE_C_UP_MODE;
		sStatusFlags |= CAM_FLAG_SMOOTH_MOVEMENT;
		sCameraSoundFlags |= CAM_SOUND_C_UP_PLAYED;
...
BAD_RETURN(s32) cutscene_mario_join_show_dialog(struct Camera *c) {
	// todo check if has entered c-up mode
    c->cutscene = 0;
    gCutsceneTimer = CUTSCENE_STOP;
	//level_set_transition(-1, NULL);
	//set_mario_action(gMarioState, ACT_READING_AUTOMATIC_DIALOG, DIALOG_033);
	canUseStageSelect = TRUE;
}
...
BAD_RETURN(s32) cutscene_exit_bowser_succ(struct Camera *c) {
    ...
    cutscene_event(cutscene_non_painting_set_cam_pos, c, 0, -1);
    cutscene_event(cutscene_non_painting_set_cam_focus, c, 0, -1);
	cutscene_event(cutscene_exit_succ_shake_landing, c, 0, -1);
}
...
BAD_RETURN(s32) cutscene_exit_painting_start(struct Camera *c) {
    ...
    vec3f_set(sCutsceneVars[2].point, 100.f, 0.f, 1189.f);
...
BAD_RETURN(s32) cutscene_exit_painting_move_to_mario(struct Camera *c) {
    ...
    //c->pos[0] = pos[0];
    //c->pos[2] = pos[2];
	//cutscene_exit_to_castle_grounds_focus_mario(c);
	Vec3f focus;
...
BAD_RETURN(s32) cutscene_door_follow_mario(struct Camera *c) {
    ...
    set_focus_rel_mario(c, 0.f, 125.f, 0.f, 0);
	vec3f_set(camOffset, 0.f, 100.f, 150.f);
	offset_rotated(finalCam, sMarioCamState->pos, camOffset, sCutsceneVars[0].angle);
    vec3f_get_dist_and_angle(c->focus, c->pos, &dist, &pitch, &yaw);
    camera_approach_f32_symmetric_bool(&dist, 150.f, 7.f);
    vec3f_set_dist_and_angle(c->focus, c->pos, dist, pitch, yaw);
	camera_approach_f32_symmetric_bool(&c->pos[0], finalCam[0], 1.25f);
	camera_approach_f32_symmetric_bool(&c->pos[1], finalCam[1], 1.25f);
	set_fov_function(CAM_FOV_SET_45);
...
// Cutscene for when Mario enters the game for the first time.
struct Cutscene sCutsceneMarioJoined[] = {
    { cutscene_mario_join_spin, 55/HURB_COEFF/*120/HURB_COEFF*/ },
    { cutscene_mario_join_switch_to_c_up, 20 },
    { cutscene_mario_join_show_dialog, 0 }
};
...
struct Cutscene sCutsceneExitSpecialSuccess[] = {
    { cutscene_exit_non_painting_succ, 120 },
    { cutscene_non_painting_end, 0 }
};
...
#define CUTSCENE(id, cutscene)                                                                            \
    ...
    switch (c->cutscene) {
        ...
        CUTSCENE(CUTSCENE_MARIO_JOINS, sCutsceneMarioJoined)
    }
...
void approach_fov_castle(UNUSED struct MarioState *m) {
    camera_approach_f32_symmetric_bool(&sFOVState.fov, 50.f, 3.5f);
}
void approach_fov_bowser(UNUSED struct MarioState *m) {
	//! Not sure of this odd choice of FOV, but it'll do
    camera_approach_f32_symmetric_bool(&sFOVState.fov, 45.f, 3.5f);
}
    ...
    if (callContext == GEO_CONTEXT_RENDER) {
        switch (fovFunc) {
            ...
            case CAM_FOV_APP_60:
                ...
                break;
            case CAM_FOV_APP_CI:
                approach_fov_castle(marioState);
                break;
            case CAM_FOV_APP_40:
                approach_fov_bowser(marioState);
                break;
            //! No default case
        }
    }
