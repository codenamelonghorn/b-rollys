...
static s32 act_water_idle(struct MarioState *m) {
    u32 val = 0x25000;
    s32 animation = m->animation;
    s16 targetYawVel = -(s16)(m->controller->stickX);
    ...
    if (m->input & INPUT_NONZERO_ANALOG) {
        ...
        m->faceAngle[1] =
            m->intendedYaw - approach_s32((s16)(m->intendedYaw - m->faceAngle[1]), 100, 0x100, 0x100);
        if (m->forwardVel < 10) {
            m->forwardVel += 5;
        }
    } else if (m->input != INPUT_NONZERO_ANALOG) {
        set_mario_animation(m, MARIO_ANIM_WATER_IDLE_MOVE_UNUSED);
    }
...
static s32 act_breaststroke(struct MarioState *m) {
	...
	//{
	//	set_mario_animation(m, sSwimStrength == MIN_SWIM_STRENGTH ? MARIO_ANIM_SWIM_PADDLE : MARIO_ANIM_SWIM_PART1);
	//}else{
...
static s32 act_swimming_end(struct MarioState *m) {
    ...
    m->forwardVel -= 0.25f;
	//if (!swimming_near_surface(m) || (sSwimStrength != MIN_SWIM_STRENGTH && m->current))
...
static s32 act_flutter_kick(struct MarioState *m) {
    ...
    if (!(m->input & INPUT_A_DOWN) || swimming_near_surface(m)) {
