...
void copy_mario_state_to_object(void) {
	...
	#if DEVELOP
	if (gShowDebugText) {
	    if ((gPlayer1Controller->buttonDown & D_JPAD) && (gPlayer1Controller->buttonDown & L_TRIG))
	    	set_mario_action(&gMarioStates[i], ACT_DEBUG_FREE_MOVE, 0);
	}
	#endif
...
void spawn_objects_from_info(UNUSED s32 unused, struct SpawnInfo *spawnInfo) {
    ...
    while (spawnInfo != NULL) {
        ...
        if ((spawnInfo->behaviorArg & (RESPAWN_INFO_DONT_RESPAWN << 8))
			...
			else if (spawnInfo->behaviorArg & 0x02) {
				//gMarioObject = object;
                gLuigiObject = object;
                geo_make_first_child(&object->header.gfx.node);
				//spawnInfo->behaviorArg = 0x00000001;
            } 
