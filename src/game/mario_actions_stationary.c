...
extern s8 gHasShownDialog33Already;
s32 check_common_idle_cancels(struct MarioState *m) {
    mario_drop_held_object(m);
    if ((gCamera->cutscene != CUTSCENE_MARIO_JOINS && gHasShownDialog33Already) || gCurrLevelNum != LEVEL_CASTLE_GROUNDS) {
	...
	if (gCamera->cutscene == CUTSCENE_MARIO_JOINS) {
...
s32 act_idle(struct MarioState *m) {
    ...
    if (m->actionState == 3) {
        ...
        } else if ((m->area->terrainType & TERRAIN_MASK) == TERRAIN_SLIDE) {
            return set_mario_action(m, ACT_OSHIN_WAIT, 0);
        } else {
...
s32 act_oshin_wait(struct MarioState *m) { // unused nausea idle
	s32 sp24;
    ...
    switch (m->actionState) {
        case 0: {
            sp24 = set_mario_animation(m, MARIO_ANIM_START_REACH_POCKET);
            if (is_anim_past_end(m)) {
                m->actionState = 1;
            }
            break;
        ...
        case 1: {
            sp24 = set_mario_animation(m, MARIO_ANIM_REACH_POCKET);
			if (is_anim_past_end(m)) {
                m->actionState = 2;
            }
            break;
...
s32 mario_execute_stationary_action(struct MarioState *m) {
    ...
    switch (m->action) {
		...
		case ACT_OSHIN_WAIT:              sp24 = act_oshin_wait(m);                       break;
