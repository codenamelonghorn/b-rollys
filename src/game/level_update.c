...
u32 pressed_pause(void) {
    ...
    u32 intangible = (gMarioStates->action & ACT_FLAG_INTANGIBLE) != 0;
...
void init_mario_after_warp(void) {
    ...
    if (gMarioStates->action != ACT_UNINITIALIZED) {
        ...
        init_mario();
        set_mario_initial_action(gMarioStates, marioSpawnType, sWarpDest.arg);

        gMarioStates->interactObj = spawnNode->object;
        gMarioStates->usedObj = spawnNode->object;
    }
    ...
    if (gCurrDemoInput == NULL) {
        ...
        if (gMarioStates->flags & MARIO_METAL_CAP) {
            play_cap_music(SEQUENCE_ARGS(4, SEQ_EVENT_METAL_CAP));
        }

        if (gMarioStates->flags & (MARIO_VANISH_CAP | MARIO_WING_CAP)) {
...
void warp_credits(void) {
    ...
    set_mario_action(gMarioStates, marioAction, 0);
...
void check_instant_warp(void) {
    ...
    if ((floor = gMarioStates->floor) != NULL) {
        ...
        if (index >= INSTANT_WARP_INDEX_START && index < INSTANT_WARP_INDEX_STOP
            ...
            if (warp->id != 0) {
                gMarioStates->pos[0] += warp->displacement[0];
                gMarioStates->pos[1] += warp->displacement[1];
                gMarioStates->pos[2] += warp->displacement[2];
                ...
                cameraAngle = gMarioStates->area->camera->yaw;
                ...
                gMarioStates->area->camera->yaw = cameraAngle;
            }
        }
    }
}
...
void initiate_painting_warp(void) {
    if (gCurrentArea->paintingWarpNodes != NULL && gMarioStates->floor != NULL) {
        struct WarpNode warpNode;
        struct WarpNode *pWarpNode = get_painting_warp_node();

        if (pWarpNode != NULL) {
            if (gMarioStates->action & ACT_FLAG_INTANGIBLE) {
    ...
    if (sDelayedWarpOp == WARP_OP_NONE) {
        ...
        switch (warpOp) {
            ...
            case WARP_OP_DEATH:
                ...
                play_transition(WARP_TRANSITION_FADE_INTO_BOWSER, 32, 0x00, 0x00, 0x00);
...
void update_hud_values(void) {
    if (gCurrCreditsEntry == NULL) {
        s16 numHealthWedges = gMarioStates->health > 0 ? gMarioStates->health >> 8 : 0;
        ...
        if (gHudDisplay.coins < gMarioStates->numCoins) {
            if (gGlobalTimer & 0x00000001) {
                u32 coinSound;
                if (gMarioStates->action & (ACT_FLAG_SWIMMING | ACT_FLAG_METAL_WATER)) {
                ...
                gHudDisplay.coins += 1;
                play_sound(coinSound, gMarioStates->marioObj->header.gfx.cameraToObject);
            }
        }

        if (gMarioStates->numLives > 100) {
            gMarioStates->numLives = 100;
        }

//#if BUGFIX_MAX_LIVES
//        if (gMarioStates->numCoins > 999) {
//            gMarioStates->numCoins = 999;
//        }
//
...
//#else
//        if (gMarioStates->numCoins > 999) {
//            gMarioStates->numLives = (s8) 999; //! Wrong variable
//        }
//#endif

        gHudDisplay.stars = gMarioStates->numStars;
        gHudDisplay.lives = gMarioStates->numLives;
        gHudDisplay.keys = gMarioStates->numKeys;
...
s32 play_mode_normal(void) {
    ...
    if (sCurrPlayMode == PLAY_MODE_NORMAL) {
        ...
        } else if (pressed_pause() && canUseStageSelect) {
...
u32 WarpArray[] = {
	LEVEL_WF,LEVEL_LLL,LEVEL_CCM,LEVEL_DDD,LEVEL_BOWSER_1
};
u32 SavedCourseNumArray[] = {
	COURSE_WF,COURSE_LLL,COURSE_CCM,COURSE_DDD,COURSE_BITDW
};

s32 play_mode_paused(void) {
#if DEVELOP
	if ((gPlayer1Controller->buttonPressed & START_BUTTON) && (gPlayer1Controller->buttonDown & B_BUTTON))  set_play_mode(PLAY_MODE_FRAME_ADVANCE);
#endif
	
    if (gPauseScreenMode == 0) {
        set_menu_mode((gCurrCourseNum >= COURSE_MIN && gCurrCourseNum <= COURSE_MAX) ? 1 : 0);//RENDER_PAUSE_SCREEN);
    } else if (gPauseScreenMode == 1) {
        raise_background_noise(1);
        //gCameraMovementFlags &= ~CAM_MOVE_PAUSE_SCREEN;
        set_play_mode(PLAY_MODE_NORMAL);
    } else if (gPauseScreenMode >= 2 && gPauseScreenMode <= 6 && !(gCurrCourseNum >= COURSE_MIN && gCurrCourseNum <= COURSE_MAX)) {
		initiate_warp(WarpArray[gPauseScreenMode - 2], 1, 0x0A, 0);
        fade_into_special_warp(0, 1);
        gSavedCourseNum = SavedCourseNumArray[gPauseScreenMode - 2];
	}
...
s32 update_level(void) {
	...
	#if DEBUGSW
	if (sCurrPlayMode != PLAY_MODE_CHANGE_LEVEL) CheckReset();
	#endif
...
s32 init_level(void) {
        ...
        if (gCurrentArea != NULL) {
            ...
            if (gCurrDemoInput != NULL) {
                ...
                if (gMarioStates->action != ACT_UNINITIALIZED) {
                    ...
                    //    set_mario_action(gMarioStates, ACT_INTRO_CUTSCENE, 0);
                    //    val4 = 1;
    ...
    if (gMarioStates->action == ACT_INTRO_CUTSCENE) {
...
s32 lvl_set_current_level(UNUSED s16 arg0, s32 levelNum) {
	...
	//if (gCurrLevelNum == LEVEL_WF) return 0; // what is this for?
    ...
    return 0;//1; // made it always 0 to hide star select
}
