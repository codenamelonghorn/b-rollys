...
void play_mario_jump_sound(struct MarioState *m) {
    if (!(m->flags & MARIO_MARIO_SOUND_PLAYED)) {
        ...
        if (m->action == ACT_TRIPLE_JUMP) {
            play_sound(SOUND_MARIO_SPINJUMP_WAHA_YIPPEE + ((gAudioRandom % 5) << 16),
...
static u32 set_mario_action_airborne(struct MarioState *m, u32 action, u32 actionArg) {
    ...
    switch (action) {
        ...
        case ACT_SIDE_FLIP:
            set_mario_y_vel_based_on_fspeed(m, /*62.0f*/42.0f, 0.0f);
            m->forwardVel = 8.0f;
 ...
 */
static u32 set_mario_action_moving(UNUSED struct MarioState *m, u32 action, UNUSED u32 actionArg) {
    //s16 floorClass = mario_get_floor_class(m);
    //f32 forwardVel = m->forwardVel;
    //f32 mag = m->intendedMag;//min(m->intendedMag, 8.0f);
...
void update_mario_joystick_inputs(struct MarioState *m) {
    ...
    f32 mag = ((controller->stickMag / 64.0f) * (controller->stickMag / 64.0f) * (controller->stickMag / 64.0f) * (controller->stickMag / 64.0f)
	* (controller->stickMag / 64.0f) * (controller->stickMag / 64.0f) * (controller->stickMag / 64.0f) * (controller->stickMag / 64.0f)) * 64.0f; // to the power of 8
...
void mario_update_hitbox_and_cap_model(struct MarioState *m) {
    ...
    if (m->invincTimer >= 3) {
        ...
        if (gGlobalTimer & 1) {
            gMarioStates->marioObj->header.gfx.node.flags |= GRAPH_RENDER_INVISIBLE;
        }
    }
...
static void debug_update_mario_cap(u16 button, s32 flags, u16 capTimer, u16 capMusic) {
    ...
    if ((gPlayer1Controller->buttonDown & Z_TRIG) && (gPlayer1Controller->buttonPressed & button)
        && ((gMarioStates->flags & flags) == 0)) {
        gMarioStates->flags |= (flags + MARIO_CAP_ON_HEAD);

        if (capTimer > gMarioStates->capTimer) {
            gMarioStates->capTimer = capTimer;
        }
...
void func_sh_8025574C(void) {
    if (gMarioStates->particleFlags & PARTICLE_HORIZONTAL_STAR) {
        queue_rumble_data(5, 80);
    } else if (gMarioStates->particleFlags & PARTICLE_VERTICAL_STAR) {
        queue_rumble_data(5, 80);
    } else if (gMarioStates->particleFlags & PARTICLE_TRIANGLE) {
        queue_rumble_data(5, 80);
    }
    if(gMarioStates->heldObj && gMarioStates->heldObj->behavior == segmented_to_virtual(bhvBobomb)) {
        reset_rumble_timers();
    }
}
#endif
...
s32 execute_mario_action(UNUSED struct Object *o) {
    ...
    if (gMarioStates->action) {
        gMarioStates->marioObj->header.gfx.node.flags &= ~GRAPH_RENDER_INVISIBLE;
        mario_reset_bodystate(gMarioStates);
        update_mario_inputs(gMarioStates);
        mario_handle_special_floors(gMarioStates);
        mario_process_interactions(gMarioStates);

        // If Mario is OOB, stop executing actions.
        if (gMarioStates->floor == NULL) {
            return 0;
        }
        ...
        while (inLoop) {
            switch (gMarioStates->action & ACT_GROUP_MASK) {
                case ACT_GROUP_STATIONARY:
                    inLoop = mario_execute_stationary_action(gMarioStates);
                    break;

                case ACT_GROUP_MOVING:
                    inLoop = mario_execute_moving_action(gMarioStates);
                    break;

                case ACT_GROUP_AIRBORNE:
                    inLoop = mario_execute_airborne_action(gMarioStates);
                    break;

                case ACT_GROUP_SUBMERGED:
                    inLoop = mario_execute_submerged_action(gMarioStates);
                    break;

                case ACT_GROUP_CUTSCENE:
                    inLoop = mario_execute_cutscene_action(gMarioStates);
                    break;

                case ACT_GROUP_AUTOMATIC:
                    inLoop = mario_execute_automatic_action(gMarioStates);
                    break;

                case ACT_GROUP_OBJECT:
                    inLoop = mario_execute_object_action(gMarioStates);
                    break;
            }
        }

        sink_mario_in_quicksand(gMarioStates);
        squish_mario_model(gMarioStates);
        set_submerged_cam_preset_and_spawn_bubbles(gMarioStates);
        update_mario_health(gMarioStates);
        update_mario_info_for_cam(gMarioStates);
        mario_update_hitbox_and_cap_model(gMarioStates);

        // Both of the wind handling portions play wind audio only in
        // non-Japanese releases.
        if (gMarioStates->floor->type == SURFACE_HORIZONTAL_WIND) {
            spawn_wind_particles(0, (gMarioStates->floor->force << 8));
#ifndef VERSION_JP
            play_sound(SOUND_ENV_WIND2, gMarioStates->marioObj->header.gfx.cameraToObject);
#endif
        }

        if (gMarioStates->floor->type == SURFACE_VERTICAL_WIND) {
        ...
        gMarioStates->marioObj->oInteractStatus = 0;
#ifdef VERSION_SH
        func_sh_8025574C();
#endif

        return gMarioStates->particleFlags;
    }
...
void init_mario(void) {
    ...
    gMarioStates->actionTimer = 0;
    gMarioStates->framesSinceA = 0xFF;
    gMarioStates->framesSinceB = 0xFF;

    gMarioStates->invincTimer = 0;

    if (save_file_get_flags()
        & (SAVE_FLAG_CAP_ON_GROUND | SAVE_FLAG_CAP_ON_KLEPTO | SAVE_FLAG_CAP_ON_UKIKI
           | SAVE_FLAG_CAP_ON_MR_BLIZZARD)) {
        gMarioStates->flags = 0;
    } else {
        gMarioStates->flags = (MARIO_CAP_ON_HEAD | MARIO_NORMAL_CAP);
    }

    gMarioStates->forwardVel = 0.0f;
    gMarioStates->squishTimer = 0;

    gMarioStates->hurtCounter = 0;
    gMarioStates->healCounter = 0;

    gMarioStates->capTimer = 0;
    gMarioStates->quicksandDepth = 0.0f;

    gMarioStates->heldObj = NULL;
    gMarioStates->riddenObj = NULL;
    gMarioStates->usedObj = NULL;

    gMarioStates->waterLevel =
        find_water_level(gPlayerSpawnInfos->startPos[0], gPlayerSpawnInfos->startPos[2]);

    gMarioStates->area = gCurrentArea;
    gMarioStates->marioObj = gMarioObject;
    gMarioStates->marioObj->header.gfx.unk38.animID = -1;
    vec3s_copy(gMarioStates->faceAngle, gMarioSpawnInfo->startAngle);
    vec3s_set(gMarioStates->angleVel, 0, 0, 0);
    vec3s_to_vec3f(gMarioStates->pos, gMarioSpawnInfo->startPos);
    vec3f_set(gMarioStates->vel, 0, 0, 0);
    gMarioStates->floorHeight =
        find_floor(gMarioStates->pos[0], gMarioStates->pos[1], gMarioStates->pos[2], &gMarioStates->floor);

    if (gMarioStates->pos[1] < gMarioStates->floorHeight) {
        gMarioStates->pos[1] = gMarioStates->floorHeight;
    }

    gMarioStates->marioObj->header.gfx.pos[1] = gMarioStates->pos[1];

    gMarioStates->action =
        (gMarioStates->pos[1] <= (gMarioStates->waterLevel - 100)) ? ACT_WATER_IDLE : ACT_IDLE;

    mario_reset_bodystate(gMarioStates);
    update_mario_info_for_cam(gMarioStates);
    gMarioStates->marioBodyState->punchState = 0;

    gMarioStates->marioObj->oPosX = gMarioStates->pos[0];
    gMarioStates->marioObj->oPosY = gMarioStates->pos[1];
    gMarioStates->marioObj->oPosZ = gMarioStates->pos[2];

    gMarioStates->marioObj->oMoveAnglePitch = gMarioStates->faceAngle[0];
    gMarioStates->marioObj->oMoveAngleYaw = gMarioStates->faceAngle[1];
    gMarioStates->marioObj->oMoveAngleRoll = gMarioStates->faceAngle[2];

    vec3f_copy(gMarioStates->marioObj->header.gfx.pos, gMarioStates->pos);
    vec3s_set(gMarioStates->marioObj->header.gfx.angle, 0, gMarioStates->faceAngle[1], 0);

    if (save_file_get_cap_pos(capPos)) {
        capObject = spawn_object(gMarioStates->marioObj, MODEL_MARIOS_CAP, bhvNormalCap);
...
void init_mario_from_save_file(void) {
    gMarioStates->unk00 = 0;
    gMarioStates->flags = 0;
    gMarioStates->action = 0;
    gMarioStates->spawnInfo = &gPlayerSpawnInfos[0];
    gMarioStates->statusForCamera = &gPlayerCameraState[0];
    gMarioStates->marioBodyState = &gBodyStates[0];
    gMarioStates->controller = &gControllers[0];
    gMarioStates->animation = &D_80339D10;

    //gMarioStates->numCoins = 0;
    gMarioStates->numStars =
        save_file_get_total_star_count(gCurrSaveFileNum - 1, COURSE_MIN - 1, COURSE_MAX - 1);
    gMarioStates->numKeys = 0;

    gMarioStates->numLives = 2;//4;
    gMarioStates->health = 0x880;

    gMarioStates->prevNumStarsForDialog = gMarioStates->numStars;
    gMarioStates->unkB0 = 0xBD;
