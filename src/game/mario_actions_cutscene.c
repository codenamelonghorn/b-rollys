...
s32 mario_ready_to_speak(void) {
    u32 actionGroup = gMarioStates->action & ACT_GROUP_MASK;
    s32 isReadyToSpeak = FALSE;

    if ((gMarioStates->action == ACT_WAITING_FOR_DIALOG || actionGroup == ACT_GROUP_STATIONARY
         || actionGroup == ACT_GROUP_MOVING)
        && (!(gMarioStates->action & (ACT_FLAG_RIDING_SHELL | ACT_FLAG_INVULNERABLE))
            && gMarioStates->action != ACT_FIRST_PERSON)) {
        isReadyToSpeak = TRUE;
    }
...
s32 set_mario_npc_dialog(s32 actionArg) {
    ...
    // in dialog
    if (gMarioStates->action == ACT_READING_NPC_DIALOG) {
        if (gMarioStates->actionState < 8) {
            dialogState = 1; // starting dialog
        }
        if (gMarioStates->actionState == 8) {
            if (actionArg == 0) {
                gMarioStates->actionState++; // exit dialog
            } else {
                dialogState = 2;
            }
        }
    } else if (actionArg != 0 && mario_ready_to_speak()) {
        gMarioStates->usedObj = gCurrentObject;
        set_mario_action(gMarioStates, ACT_READING_NPC_DIALOG, actionArg);
        dialogState = 1; // starting dialog
    }
...
s32 act_quicksand_death(struct MarioState *m) {
    ...
    if (m->actionState == 1) {
        ...
        //    play_sound_if_no_flag(m, SOUND_MARIO_ON_FIRE, MARIO_ACTION_SOUND_PLAYED);
        //}
...
s32 act_spawn_spin_airborne(struct MarioState *m) {
    ...
    if(m->vel[1] > 0 || m->actionArg == 0){//if (m->actionState == 0 && m->pos[1] - m->floorHeight > 300.0f) {
...
s32 act_exit_airborne(struct MarioState *m) {
	set_camera_mode(gCamera, CAMERA_MODE_FREE_ROAM, 0);//! TODO
    if (15 < m->actionTimer++
...
s32 act_death_exit(struct MarioState *m) {
    ...
    if (15 < m->actionTimer++
		...
		play_sound(SOUND_MARIO_SECONDJUMP, m->marioObj->header.gfx.cameraToObject);
#else
...
static s32 jumbo_star_cutscene_taking_off(struct MarioState *m) {
    ...
    if (m->actionState == 0) {
        ...
        switch (animFrame) {
            ...
            case 60:
                play_sound(SOUND_MARIO_SPINJUMP, m->marioObj->header.gfx.cameraToObject);
                break;
        }
        m->particleFlags |= PARTICLE_SPARKLES;
