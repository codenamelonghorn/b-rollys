...
void bowser_spawn_grand_star_key(void) {
    ...
    if (BITS)
        ...
        /*gSecondCameraFocus =*/ spawn_object_relative(0, -250, 250, 250, o, MODEL_STAR, bhvStar);//MODEL_BOWSER_KEY, bhvBowserKey);
        //cur_obj_play_sound_2(SOUND_GENERAL2_BOWSER_KEY);
		obj_spawn_loot_yellow_coins(o, 20, 0.f);
    }
	gSecondCameraFocus = spawn_object(o, MODEL_NONE, bhvUnused05A8); // set focus to dummy object so the camera doesn't freak out after Bowser despawns
...
s32 bowser_dead_twirl_into_trophy(void) {
    ...
    //if (o->header.gfx.scale[0] > 0.2) {
        o->header.gfx.scale[0] = o->header.gfx.scale[0] - 0.025;
		o->header.gfx.scale[1] = o->header.gfx.scale[1] - 0.025;
        o->header.gfx.scale[2] = o->header.gfx.scale[2] - 0.025;
    /*} else {
        o->header.gfx.scale[1] = o->header.gfx.scale[1] - 0.01;
        o->oVelY = 20.0f;
        o->oGravity = 0.0f;
    }*/
    if (o->header.gfx.scale[1] < 0.1)//.5)
        ret = 1;
