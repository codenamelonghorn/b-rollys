...
void play_far_fall_sound(struct MarioState *m) {
    ...
    if (!(action & ACT_FLAG_INVULNERABLE) && action != ACT_TWIRLING && action != ACT_FLYING
        ...
        if (m->peakHeight - m->pos[1] > 1150.0f) {
			...
			play_sound(SOUND_MARIO_FALL, m->marioObj->header.gfx.cameraToObject); // actually falling
            m->flags |= MARIO_UNKNOWN_18;
        }
    }
}
...
void play_knockback_sound(struct MarioState *m) {
    if (m->actionArg == 0 && (m->forwardVel <= -28.0f || m->forwardVel >= 28.0f)) {
        ...
        play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, SOUND_MARIO_SECONDJUMP);
    }
}
#endif
...
s32 lava_boost_on_wall(struct MarioState *m) {
    ...
    //play_sound(SOUND_MARIO_ON_FIRE, m->marioObj->header.gfx.cameraToObject);
	play_sound(SOUND_MARIO_DAMAGE, m->marioObj->header.gfx.cameraToObject);
...
s32 check_kick_or_dive_in_air(struct MarioState *m) {
    if (m->input & INPUT_B_PRESSED) {
        return set_mario_action(m, ACT_DIVE, 0);//set_mario_action(m, m->forwardVel > 28.0f ? ACT_DIVE : ACT_JUMP_KICK, 0);
    }
    return FALSE;
}
...
#else
        //play_sound(SOUND_MARIO_TUMBLE, m->marioObj->header.gfx.cameraToObject);
#endif
...
s32 act_triple_jump(struct MarioState *m) {
    ...
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, 0);
#else
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, SOUND_MARIO_SPINJUMP);
#endif
...
s32 act_side_flip(struct MarioState *m) {
    ...
    if (common_air_action_step(m, ACT_SIDE_FLIP_LAND, MARIO_ANIM_SLIDEFLIP, AIR_STEP_CHECK_LEDGE_GRAB)
        ...
        m->marioObj->header.gfx.angle[1] += (0x4000)+((0x8000)/28 * m->marioObj->header.gfx.unk38.animFrame);
		//m->marioObj->header.gfx.angle[2] += m->marioObj->header.gfx.unk38.animFrame * 3584;
    }
...
s32 act_long_jump(struct MarioState *m) {
    ...
    if (m->floor->type == SURFACE_VERTICAL_WIND && m->actionState == 0) {
        play_sound(SOUND_MARIO_YEEAH, m->marioObj->header.gfx.cameraToObject);
        m->actionState = 1;
    }
...
s32 act_dive(struct MarioState *m) {
    if (m->actionArg == 0) {
        play_mario_sound(m, SOUND_ACTION_THROW, SOUND_MARIO_SECONDJUMP);
    } else {
        play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, 0);
    }
...
s32 act_air_throw(struct MarioState *m) {
    ...
    play_sound_if_no_flag(m, SOUND_MARIO_THROW, MARIO_MARIO_SOUND_PLAYED);
...
s32 act_ground_pound(struct MarioState *m) {
    ...
    if (m->actionState == 0 && stepResult != AIR_STEP_LANDED) {
        ...
        if (m->actionTimer >= m->marioObj->header.gfx.unk38.curAnim->unk08 + 4) {
            play_sound(/*SOUND_MARIO_GROUND_POUND_WAH*/SOUND_MARIO_SECONDJUMP, m->marioObj->header.gfx.cameraToObject);
            m->actionState = 1;
			m->vel[1] = -50.0f;
        }
    } else {
...
s32 act_thrown_backward(struct MarioState *m) {
	...
	play_sound_if_no_flag(m, SOUND_MARIO_DAMAGE, MARIO_MARIO_SOUND_PLAYED);
...
s32 act_soft_bonk(struct MarioState *m) {
    ...
    play_knockback_sound(m);
#else
    //play_sound_if_no_flag(m, SOUND_MARIO_FALL, MARIO_MARIO_SOUND_PLAYED);
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, SOUND_MARIO_SECONDJUMP);
#endif
        ...
        case AIR_STEP_LANDED:
            if (m->floor->type == SURFACE_BURNING) {
                ...
                if (m->actionState < 2 && m->vel[1] < 0.0f) {
					m->vel[1] = -m->vel[1] * .6f; //begtah bounce from lava myes, thx godly
...
s32 act_shot_from_cannon(struct MarioState *m) {
    ...
    play_sound_if_no_flag(m, SOUND_MARIO_SPINJUMP, MARIO_MARIO_SOUND_PLAYED);
    ...
    if (/*(m->flags & MARIO_WING_CAP) &&*//*m->actionTimer++ > 30 ||*/ m->vel[1] < 0.0f) {
...
s32 act_flying(struct MarioState *m) {
    ...
    if (!(m->flags & MARIO_WING_CAP || m->actionArg == 0)) {
        if (m->area->camera->mode == CAMERA_MODE_BEHIND_MARIO) {
            set_camera_mode(m->area->camera, m->area->camera->defMode, 1);
        }
...
s32 act_vertical_wind(struct MarioState *m) {
    ...
    play_sound_if_no_flag(m, SOUND_MARIO_YEEAH, MARIO_MARIO_SOUND_PLAYED);
    if (m->actionState == 0) {
        set_mario_animation(m, MARIO_ANIM_FORWARD_SPINNING_FLIP);
        if (m->marioObj->header.gfx.unk38.animFrame == 1) {
            play_sound(SOUND_ACTION_SPIN, m->marioObj->header.gfx.cameraToObject);
