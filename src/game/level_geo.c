...
Gfx *geo_skybox_main(s32 callContext, struct GraphNode *node, UNUSED Mat4 *mtx) {
    ...
    if (callContext == GEO_CONTEXT_AREA_LOAD) {
		...
		struct Camera *cam = camNode->config.camera;
		...
		//! tried and true SW-style skybox scroll
        gfx = create_skybox_facing_camera(0, backgroundNode->background, camFrustum->fov,
					...
					//gLakituState.goalPos[0], gLakituState.goalPos[1], gLakituState.goalPos[2], 
					...
					//gLakituState.goalFocus[0], gLakituState.goalFocus[1], gLakituState.goalFocus[2]
