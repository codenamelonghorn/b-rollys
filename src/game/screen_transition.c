...
extern s8 wipe_star_txt[];
extern s8 wipe_circle_txt[];
extern s8 wipe_docro_txt[];
extern s8 wipe_mario_txt[];
extern s8 wipe_kopa_txt[];
...
s32 render_textured_transition(s8 fadeTimer, s8 transTime, struct WarpTransitionData *transData, s8 texID, s8 transTexType) {
	...
	f32 transTime2 = transTime;
	texTransRadius = calc_tex_transition_radius(fadeTimer, transTime2, transData);
