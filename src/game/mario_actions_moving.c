...
void apply_slope_accel(struct MarioState *m) {
    ...
    if (mario_floor_is_slope(m)) {
        ...
        switch (slopeClass) {
            case SURFACE_CLASS_VERY_SLIPPERY:
                slopeAccel = 2.1f;//5.3f;
                break;
            case SURFACE_CLASS_SLIPPERY:
                slopeAccel = 1.4f;//2.7f;
                break;
            default:
                slopeAccel = 0.9f;//1.7f;
                break;
            case SURFACE_CLASS_NOT_SLIPPERY:
                slopeAccel = 0.0f;
                break;
...
void update_walking_speed(struct MarioState *m) {
    ...
    if (m->forwardVel <= 0.0f) {
        ...
        m->forwardVel += .8f - m->forwardVel / 43.0f;
    } else if (m->floor->normal.y >= 0.95f) {
        m->forwardVel -= .8f;
    }
...
void anim_and_audio_for_walk(struct MarioState *m) {
    ...
    if (m->quicksandDepth > 50.0f) {
        val14 = (s32)(val04 * 0x4000);/// 4.0f * 0x10000);
            ...
            switch (m->actionTimer) {
                case 0:
                    if (val04 > 8.0f) {
                        ...
                        //if ((val14 = (s32)(val04 * 0x4000/*/ 4.0f * 0x10000*/)) < 0x1000) {
                        //    val14 = 0x1000;
                        //}
						val14 = (s32)(val04 * 0x4000);
...
s32 act_decelerating(struct MarioState *m) {
    ...
    if (!(m->input & INPUT_FIRST_PERSON)) {
        ...
        if (m->input & INPUT_NONZERO_ANALOG) {
            //return set_mario_action(m, ACT_WALKING, 0);
			if (m->forwardVel < 10.0f) mario_set_forward_vel(m, 0.0f);
        }
...
s32 act_crouch_slide(struct MarioState *m) {
    ...
    //if (m->input & INPUT_B_PRESSED) {
        ...
        //} else {
            //return set_mario_action(m, ACT_MOVE_PUNCHING, 0x0004);
        //}
    //}
...
//#else
//        play_sound_if_no_flag(m, SOUND_MARIO_TUMBLE, MARIO_MARIO_SOUND_PLAYED);
//#endif
//    }
...
s32 act_long_jump_land(struct MarioState *m) {
    ...
    /*if (!(m->input & INPUT_NONZERO_ANALOG)) {
        play_sound_if_no_flag(m, SOUND_MARIO_FALL2_2, MARIO_MARIO_SOUND_PLAYED);
    }*/
