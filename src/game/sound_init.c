...
void play_infinite_stairs_music(void) {
    ...
    /* Infinite stairs? */
    if (gCurrLevelNum == LEVEL_CASTLE && gCurrAreaIndex == 2 && gMarioStates->numStars < 70) {
        if (gMarioStates->floor != NULL && gMarioStates->floor->room == 6) {
            if (gMarioStates->pos[2] < 2540.0f) {
                shouldPlay = TRUE;
            }
        }
    }
