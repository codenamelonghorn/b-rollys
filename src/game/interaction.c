...
u32 mario_lose_cap_to_enemy(u32 arg) {
    ...
    if (does_mario_have_hat(gMarioStates)) {
        save_file_set_flags(arg == 1 ? SAVE_FLAG_CAP_ON_KLEPTO : SAVE_FLAG_CAP_ON_UKIKI);
        gMarioStates->flags &= ~(MARIO_NORMAL_CAP | MARIO_CAP_ON_HEAD);
        wasWearingCap = TRUE;
    }
...
void mario_retrieve_cap(void) {
    ...
    gMarioStates->flags &= ~MARIO_CAP_ON_HEAD;
    gMarioStates->flags |= MARIO_NORMAL_CAP | MARIO_CAP_IN_HAND;
}
...
u32 determine_knockback_action(struct MarioState *m, UNUSED s32 arg) {
    ...
    if (terrainIndex == 2) {
        if (m->forwardVel < 28.0f && terrainIndex != 2) {
        ...
        if (m->forwardVel < 16.0f && terrainIndex != 2) {
	...
	if (terrainIndex == 2) mario_set_forward_vel(m, 0);
    return bonkAction;
}
...
u32 interact_coin(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (m->numCoins - o->oDamageOrCoinValue < 100
        && m->numCoins >= 100) {
		play_sound(SOUND_GENERAL_COLLECT_1UP, gDefaultSoundArgs);
        gMarioStates->numLives++;
		m->numCoins = 0;
		gHudDisplay.coins = 0;
    }
#ifdef VERSION_SH
...
u32 interact_star_or_key(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (m->health >= 0x100) {
        ...
        spawn_object(o, MODEL_ITEM_STAR_DUST, bhvStarKeyCollectionPuffSpawner);
...
u32 interact_flame(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    u32 burningAction = ACT_BURNING_GROUND;//ACT_BURNING_JUMP;
...
u32 interact_bully(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (interaction & INT_ATTACK_NOT_FROM_BELOW) {
        ...
        update_mario_sound_and_camera(m);
        play_sound(SOUND_MARIO_CLIMB, m->marioObj->header.gfx.cameraToObject);
...
void check_death_barrier(struct MarioState *m) {
    if (m->pos[1] < m->floorHeight + 2048.0f) {
        if (level_trigger_warp(m, WARP_OP_WARP_FLOOR) == 20 && !(m->flags & MARIO_UNKNOWN_18)) {
            play_sound(SOUND_MARIO_FALL, m->marioObj->header.gfx.cameraToObject);
        }
    }
}
