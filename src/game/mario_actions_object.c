...
s32 mario_update_punch_sequence(struct MarioState *m) {
    ...
    switch (m->actionArg) {
        ...
        case 1:
			...
			if (is_anim_past_end(m)/*animFrame > 7*/) {
...
s32 act_picking_up(struct MarioState *m) {
    ...
    if (m->actionState == 0) {
        ...
        play_sound_if_no_flag(m, SOUND_MARIO_LIFT, MARIO_MARIO_SOUND_PLAYED);
        m->actionState = 1;
    }
...
#endif
        play_sound(SOUND_MARIO_LIFT, m->marioObj->header.gfx.cameraToObject);
    }
